...
//timoni:generate timoni mod vendor crd -f /tmp/kue/cluster-api-provider-aws/config/crd/bases/infrastructure.cluster.x-k8s.io_awsclusterroleidentities.yaml

package v1beta2

import "strings"

// AWSClusterRoleIdentity is the Schema for the
// awsclusterroleidentities API
...
#AWSClusterRoleIdentitySpec: {
	...
	// The duration, in seconds, of the role session before it is
	// renewed.
	durationSeconds?: uint & >=900 & <=43200

	// A unique identifier that might be required when you assume a
	// role in another account.
	// If the administrator of the account to which the role belongs
	// provided you with an
	// external ID, then provide that value in the ExternalId
	// parameter. This value can be
	// any string, such as a passphrase or account number. A
	// cross-account role is usually
	// set up to trust everyone in an account. Therefore, the
	// administrator of the trusting
	// account might send an external ID to the administrator of the
	// trusted account. That
	// way, only someone with the ID can assume the role, rather than
	// everyone in the
	// account. For more information about the external ID, see How to
	// Use an External ID
	// When Granting Access to Your AWS Resources to a Third Party in
	// the IAM User Guide.
	externalID?: string

	// An IAM policy as a JSON-encoded string that you want to use as
	...
	// SourceIdentityRef is a reference to another identity which will
	// be chained to do
