...
func NewElasticsearchConnector(ctx context.Context,
	...
	// based on whether we're running in development mode or not.
	...
	client, err := elasticsearch.NewClient(elasticsearch.Config{
		...
		EnableMetrics:     true,
		//
		// The global trace-provider is used.
...
func (e *ElasticsearchConnector) Healthcheck() error {
	...
	if err != nil {
		return utils.WrapErrorWithPrefix("Failed pinging Elasticsearch cluster", err)
	}
	if response.StatusCode != http.StatusOK {
		return utils.WrapErrorWithPrefix(
