...
import (
	...
	"github.com/Archisman-Mridha/x-clone/backend/microservices/profiles/internal/config"
	"github.com/Archisman-Mridha/x-clone/backend/microservices/profiles/internal/constants"
	"github.com/Archisman-Mridha/x-clone/backend/microservices/profiles/internal/profiles"
...
func run(ctx context.Context, config *config.Config, validator *validator.Validate) error {
	...
	elasticsearchConnector := connectors.NewElasticsearchConnector(ctx,
		&connectors.NewElasticsearchConnectorArgs{
			NodeAddresses: config.Elasticsearch.NodeAddresses,
	...
	defer elasticsearchConnector.Shutdown()

	// Construct services.

	profilesService := profiles.NewProfilesService(
		validator,
		profiles.NewProfilesPostgresRepository(postgresConnector),
		profiles.NewProfilesElasticsearchSearchEngine(elasticsearchConnector),
	)
	...
	profilesAPI := profiles.NewProfilesAPI(profilesService)
	generated.RegisterProfilesAPIServer(gRPCServer, profilesAPI)
