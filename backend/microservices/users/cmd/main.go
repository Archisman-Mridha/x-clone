...
import (
	...
	"github.com/Archisman-Mridha/x-clone/backend/microservices/users/internal/config"
	"github.com/Archisman-Mridha/x-clone/backend/microservices/users/internal/constants"
	apiErrors "github.com/Archisman-Mridha/x-clone/backend/microservices/users/internal/errors"
	"github.com/Archisman-Mridha/x-clone/backend/microservices/users/internal/token"
	"github.com/Archisman-Mridha/x-clone/backend/microservices/users/internal/users"
...
func run(ctx context.Context, config *config.Config, validator *validator.Validate) error {
	...
	usersService := users.NewUsersService(
		validator,
		users.NewUsersPostgresRepository(postgresConnector),
	...
	generated.RegisterUsersAPIServer(gRPCServer, usersAPI)
...
func getGRPCErrorStatusCode(err error) codes.Code {
	...
	switch apiErr {
	case apiErrors.ErrInvalidEmail, apiErrors.ErrInvalidUsername:
		return codes.InvalidArgument

	case apiErrors.ErrDuplicateEmail, apiErrors.ErrDuplicateUsername:
		return codes.AlreadyExists

	case apiErrors.ErrInvalidJWT, apiErrors.ErrExpiredJWT:
		return codes.Unauthenticated

	case apiErrors.ErrUserNotFound:
